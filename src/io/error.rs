...
pub enum ErrorKind {
    ...
    TimedOut,
    /// An operation could not be completed because the stream reached its
    /// end-of-file equivalent.
    ///
    /// This error is *not* used by `read` and `write` where `Ok(0)` signals
    /// that the end of file has been reached but rather it is used by functions
    /// such as `write_all` where it is considered an error if the entire
    /// operation did not complete at once.
    EndOfFile,
...
impl Error {
    ...
    #[unstable = "the arguments to this function may change"]
